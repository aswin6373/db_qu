...
export function Shell({ active, onActive, onLogout, orgName, children }: Props) {
  ...
  return (
    <div className="min-h-screen bg-canvas">
      {/* Desktop sidebar */}
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col bg-slate-950 p-5 shadow-sidebar lg:flex">
        <Brand />
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-40 lg:hidden" role="dialog" aria-modal="true">
          ...
          <aside className="absolute inset-y-0 left-0 flex w-72 max-w-[85vw] flex-col bg-slate-950 p-5 shadow-lift">
            <div className="flex items-center justify-between">
              <Brand />
              <button aria-label="Close menu" className="rounded-lg p-2 text-slate-400 transition hover:bg-white/10 hover:text-white" onClick={() => setDrawerOpen(false)} type="button">
                <X size={20} />
              </button>
            </div>
            {orgName && <OrgChip name={orgName} />}
            ...
            <button className="btn-ghost w-full justify-start text-slate-400 hover:bg-white/5 hover:text-white" onClick={onLogout} type="button">
              <LogOut size={16} /> Sign out
            </button>
          </aside>
        </div>
      ...
      <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/85 backdrop-blur lg:hidden">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-3 px-4">
          ...
          <strong className="text-sm font-semibold text-slate-900">{current?.label ?? "QueryMind"}</strong>
      ...
      <main className="lg:pl-64">
        <div className="sticky top-0 z-10 hidden h-14 items-center border-b border-slate-200/80 bg-white/70 backdrop-blur lg:flex">
          <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-8">
            ...
            {orgName && <p className="text-xs font-medium text-slate-400">Workspace · {orgName}</p>}
          </div>
        </div>
...
function Brand() {
  return (
    <div className="flex items-center gap-3">
      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-brand-500 to-violet-600 text-white shadow-lg shadow-brand-600/25">
        <Database size={19} />
      </span>
      <div>
