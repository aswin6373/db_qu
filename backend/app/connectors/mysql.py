...
class MySQLConnector(DBConnector):
    ...
    def tunneled_config(self) -> Iterator[dict[str, Any]]:
        """Yield the connection config, routing through the SSH tunnel when
        configured. The tunnel is opened once per connector instance and stays
        up until close() so a multi-statement request does not re-handshake
        for every statement."""
