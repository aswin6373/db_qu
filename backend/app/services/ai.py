...
def _normalize_gemini_model(model: str | None) -> str:
    ...
    if not m or m.lower() == "default":
        return get_settings().gemini_model or "gemini-3.6-flash"
    return m
...
def _effective_ai(config: AIConfig | None) -> _EffectiveAI:
    ...
    return _EffectiveAI(
        ...
        gemini_model=_normalize_gemini_model(org_model if (org_provider == "gemini" and org_model) else getattr(settings, "gemini_model", "gemini-3.6-flash")),
